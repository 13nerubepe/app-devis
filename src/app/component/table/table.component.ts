...
export class TableComponent implements OnInit{
  ...
  getLoadDevis(): void{
    ...
    //      this.devis = data;
    //    });
    this.proformasService.getDevis().subscribe({
      next: (value) => {
        // linitialisation de client
        this.devis = value
      },
  ...
  load($event: TableLazyLoadEvent) {
   ...
   selectionClient(devis: Devis):void {
    ...
    const selectedClient = this.client.find(client => client.clientId === devis.client.clientId);
    if (selectedClient) {
  ...
  selectionProduct(devis: Devis):void {
     const selectedProduct = this.topSelling.find(product =>product.productId ===devis.product.productId)
    if(selectedProduct){
  ...
  onQuantityChange(devis: Devis, event: any): void {
    if(devis.product){
      ...
      if (typeof devis.product.qte === 'number') {
        ...
        this.proformasService.updatequantite(devis.product.productId, devis.product.qte).subscribe();
      }
    }
  }
